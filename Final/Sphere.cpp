...
Ray Sphere::getNormal(Ray point)
{
...
double Sphere::findIntersection(Line line)
{
	...
	double a = 1;
	double b = (2 * (lineOriginX - sphereCenterX) * lineDirectionX) + 
		(2 * (lineOriginY - sphereCenterY) * lineDirectionY) + 
		(2 * (lineOriginZ - sphereCenterZ) * lineDirectionZ);
	double c = pow (lineOriginX - sphereCenterX, 2) + 
		pow (lineOriginY - sphereCenterY, 2) +
		pow (lineOriginZ - sphereCenterZ, 2) - (_radius * _radius);
